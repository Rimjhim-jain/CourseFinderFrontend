...
import { deleteUser, getAllUsers, updateUserRole } from '../../../redux/actions/admin.js';
              ...
              <Tbody>
                  {
                    users && users.map(item => (
                      <Row updateHandler={updateHandler} deleteButtonHandler={deleteButtonHandler} key={item._id} item={item} loading={loading}/>
                    ))
                  }
              </Tbody>
            </Table>
    ...
    <Tr>
      ...
      <Td isNumeric>
        <HStack justifyContent ={"flex-end"}>
          <Button onClick={()=>updateHandler(item._id)} variant={"outline"} color={"purple.500"} isLoading={loading}>Change Role</Button>
